...
namespace Columbia583.Android
{
	...
	public class SearchTrailsActivity : AndroidActivity
	{
		...
		protected override void OnCreate (Bundle bundle)
		{
			...
			// still in debug state
			List<SearchResult> debugSearchResults = dataLayer.getTrailsBySearchFilter (new SearchFilter (){ rating = 1 });
			List<Activity> debugActivities = dataLayer.getActivities ();
			List<Amenity> debugAmenities = dataLayer.getAmenities ();

			//Trail[] debugTrails = new Trail[dataLayer.getTrailsBySearchFilter(new SearchFilter(){}).Count];
			//Activity[] debugActivities = new Activity[dataLayer.getActivities().Count];
			//Amenity[] debugAmenities = new Amenity[dataLayer.getAmenities().Count];
...
//						// TODO: Display the trail name.
						...
						trailElements[0].Text = result.trail.name;
//
						// TODO: Display the trail rating.
						trailElements[1] = new TextView (this);
						string ratingStars = "";
						for (int i = 0; i < result.trail.rating; i++) {
							ratingStars += "*";
						}
...
//						// TODO: Display the trail difficulty.
						...
						trailElements[2].Text = result.trail.difficulty.ToString().Replace("_", " ");
//
//						// TODO: Display the trail distance.
						trailElements[3] = new TextView (this);
						trailElements[3].Text = result.trail.distance + " km";
						...
						for (int i = 0; i < NUM_ELEMENTS_PER_TRAIL; i++) {
							trailElements[i].Click += (sender, e) => {
								...
								var intent = new Intent (this, typeof(ViewTrailActivity));
								// JSON serialization works
								string trailJSONStr = Newtonsoft.Json.JsonConvert.SerializeObject (result.trail);
								string activitiesJSONStr = Newtonsoft.Json.JsonConvert.SerializeObject(result.activities);
								string amenitiesJSONStr = Newtonsoft.Json.JsonConvert.SerializeObject(result.amenities);
								intent.PutExtra ("viewedTrail", trailJSONStr);
								intent.PutExtra("viewedActivities", activitiesJSONStr);
								intent.PutExtra("viewedAmenities", amenitiesJSONStr);
								StartActivity (intent);
