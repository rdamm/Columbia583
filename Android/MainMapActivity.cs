...
namespace Columbia583.Android
{
	...
	public class MainMapActivity : FormsApplicationActivity
	{
		...
		private void getTrailHeads()
		{
			...
			if (trails != null) {
				foreach (SearchResult trail in trails) {
					searchKMLs.Add (getKMLString (trails [index].trail.name));
					index++;
				}
			} else {
				foreach (SearchResult trail in debugSearchResults) {
					searchKMLs.Add (getKMLString (debugSearchResults [index].trail.name));
					index++;
				}
			}

			int traverseLength = searchKMLs.Count;
