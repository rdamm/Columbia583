...
namespace Columbia583
{
	public class Trail : IEquatable<Trail>
	{
		...
		public Trail (int id, int userId, int orgId, string name, string location, string kmlName, string kmlContent, string distance, string duration, string description, string directions,
			Difficulty difficulty, int rating, string hazards, string surface, string landAccess, string maintenance, string season, bool open, bool active, DateTime createdAt, DateTime updatedAt,
			bool pushToServer)
		{
			this.id = id;
			this.userId = userId;
